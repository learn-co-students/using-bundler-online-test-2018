...
describe 'Bundler' do
  before :all do
    @bundle_output = ''
    Bundler.with_clean_env { @bundle_output = `bundle` }
  end
  ...
  describe 'Gemfile' do
    ...
    it 'should specify rubygems as a source using the SSL protocol on the first line' do
      expect(
        @gemfile_text =~ %r{source ['"]https:\/\/rubygems\.org['"]}
      ).not_to eq(nil)
    end
    ...
    it 'should list the sinatra gem with the specific version 1.4.4' do
      expect(
        @gemfile_text =~ /gem ['"]sinatra['"], ?['"]v?1\.4\.4['"]/
      ).not_to eq(nil)
    end
    ...
    it 'should list the awesome_print gem specifying a remote git repository (use github)' do
      expect(
        @gemfile_text =~
          %r{gem ['"]awesome_print['"], ?(git:|:git ?=>) ?['"]git@github\.com:awesome\-print\/awesome_print\.git['"]}
      ).not_to eq(nil)
    end
    ...
    describe 'groups' do
      ...
      it 'should contain the pry gem in the development group using a hash argument to the gem method' do
        expect(@gemfile_text).to match(
          /gem ['"]pry['"], ?(group:|:group ?=>) ?(:development|['"]development['"])/
        ) |
          match(
            /group ?(:development|['"]development['"])[\s\S]*gem ['"]pry['"][\s\S]*end/
          )
  ...
  describe 'integration' do
    ...
    it 'should require bundler/setup in the environment' do
      expect(@environment_text =~ %r{require ['"]bundler\/setup['"]}).not_to eq(
                                                                              nil
                                                                            )
    end
    ...
    it 'should require the default and development groups in the environment' do
      expect(
        @environment_text =~ /Bundler\.require\(:default, :development\)/
      ).not_to eq(nil)
    end
